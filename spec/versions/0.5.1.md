...
### 4.2 Local registry format

The local registry (`registry.json`) maintains an index of discovered tools:
...
```json
{
  ...
  "tools": {
    ...
    "curl": {
      ...
      "shimSource": "https://atip.dev/shims/curl.json",
      "trustLevel": "community",
      "lastChecked": "2026-01-15T10:30:00Z"
    },
    "internal-deploy": {
      "path": "/usr/local/bin/internal-deploy",
      "version": "3.1.0",
      "source": "shim",
      "shimPath": "./shims/internal-deploy.json",
      "shimSource": "https://atip.internal.acme.com/shims/internal-deploy.json",
      "trustLevel": "org",
      "lastChecked": "2026-01-15T10:30:00Z"
    }
  },
  "statistics": {
    ...
    "nativeTools": 12,
    "shimTools": 35,
    "lastFullScan": "2026-01-10T00:00:00Z"
  }
}
```
...
| `agent` | object | Agent that created/updated registry |
| `tools` | object | Map of tool name to tool entry |
...
| `atipVersion` | string | ATIP protocol version |
| `source` | enum | `native`, `shim`, `inferred` |
| `trustLevel` | enum | `verified`, `native`, `vendor`, `org`, `community`, `user`, `inferred` |
| `lastChecked` | string | When tool was last verified |
| `metadataPath` | string | Path to cached ATIP metadata (relative to registry) |
| `shimPath` | string | Path to shim file (if source=shim) |
| `shimSource` | string | URL where shim was fetched from |
| `checksum` | string | Binary checksum for integrity verification |

### 4.3 Remote registry protocol

Remote registries provide shims for tools without native ATIP support. Registries can be static (file hosting) or dynamic (API).

#### 4.3.1 Static registry (recommended)

A static registry is a directory of JSON files served over HTTPS:
...
├── shims/
│   ├── index.json                  # Shim index
│   ├── curl.json                   # Individual shims
│   ├── curl.json.bundle            # Signature bundle
│   ├── rsync.json
│   ├── rsync.json.bundle
│   └── ...
└── patterns/
...
**Registry manifest** (`.well-known/atip-registry.json`):
...
```json
{
  ...
  "registry": {
    ...
    "type": "static",
    "version": "2026.01.15",
    "maintainer": "atip-maintainers@example.org"
  },
  "endpoints": {
    "shims": "/shims/",
    "shimsIndex": "/shims/index.json",
    "patterns": "/patterns/",
    "patternsIndex": "/patterns/index.json"
  },
  "trust": {
...
```json
{
  ...
  "shims": {
    "curl": {
      ...
      "atipVersion": "0.5",
      "path": "curl.json",
      "signaturePath": "curl.json.bundle",
    ...
    "rsync": {
      ...
      "atipVersion": "0.5",
      "path": "rsync.json",
      "signaturePath": "rsync.json.bundle",
...
#### 4.3.2 API registry (optional)

For dynamic registries, a simple REST API:

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/.well-known/atip-registry.json` | GET | Registry manifest |
| `/api/v1/shims` | GET | List all shims (paginated) |
| `/api/v1/shims/{name}` | GET | Get shim metadata |
| `/api/v1/shims/{name}/download` | GET | Download shim JSON |
| `/api/v1/search?q={query}` | GET | Search shims |
...
Agents discover registries through:

1. **Explicit configuration** (`~/.config/agent-tools/registries.json`)
2. **Well-known URLs** for domains
...
When multiple registries provide shims for the same tool:
1. Lower priority number wins (50 < 100)
2. If equal priority, org > community > inferred
3. If still equal, first configured wins
...
| `ATIP_REGISTRY_TOKEN` | Default bearer token for authenticated registries |
| `ATIP_SYNC_INTERVAL` | Override sync interval (e.g., `12h`, `7d`) |
| `ATIP_OFFLINE` | If set, skip remote registry sync |

### 4.5 Sync and updates

#### 4.5.1 Sync algorithm

```python
def sync_registries(config):
    for registry in sorted(config.registries, key=lambda r: r.priority):
        if not registry.enabled:
            continue
            
        # Fetch manifest
        manifest = fetch(f"{registry.url}/.well-known/atip-registry.json")
        
        # Check if update needed
        local_version = get_local_registry_version(registry.url)
        if manifest.registry.version <= local_version:
            continue  # Already up to date
        
        # Fetch shim index
        index = fetch(f"{registry.url}{manifest.endpoints.shimsIndex}")
        
        # Download new/updated shims
        for name, shim in index.shims.items():
            local_shim = get_local_shim(name, registry.url)
            if not local_shim or shim.checksum != local_shim.checksum:
                download_shim(registry, name, shim)
                
        # Update local registry
        update_local_registry(registry.url, manifest.registry.version)
```
...
|----------|---------------------|
| Interactive agent | On startup + every 24h |
| CI/CD environment | On each run (with cache) |
| Air-gapped system | Manual sync only |
| Enterprise managed | Per organization policy |

#### 4.5.3 Offline mode

When `ATIP_OFFLINE=1` or network unavailable:

1. Use only cached shims from local registry
2. Skip remote registry sync
3. Log warning if requested shim not in cache
4. Allow explicit `--sync` flag to override

### 4.6 Organization registries

Enterprises can run private ATIP registries with additional controls.

#### 4.6.1 Deployment options

| Option | Description | Complexity |
|--------|-------------|------------|
| Static files on CDN | S3/GCS/Cloudflare | Low |
| Static files on internal server | nginx/Apache | Low |
| API server | Custom implementation | Medium |
| Artifact registry integration | Artifactory/Nexus plugin | Medium |

#### 4.6.2 Organization policy file

Organizations can enforce policies via a policy file in the registry:
...
```json
{
  "policy": {
    ...
    "toolApprovals": {
      "mode": "allowlist",
      "approved": ["gh", "kubectl", "terraform", "internal-*"],
      "blocked": ["rm", "dd", "mkfs.*"],
      "requireApprovalFor": {
        "destructive": true,
        "billable": true,
...
```json
{
  "auth": {
    "type": "mtls",
    "certPath": "~/.config/agent-tools/certs/client.crt",
    "keyPath": "~/.config/agent-tools/certs/client.key"
  }
}
```

### 4.7 Shim contribution workflow

For community registry contributions:
...
# 2. Validate schema
atip validate ffmpeg.json

# 3. Sign shim (maintainers only)
cosign sign-blob --yes \
  --output-signature ffmpeg.json.sig \
  --output-certificate ffmpeg.json.crt \
  ffmpeg.json

cosign bundle create \
  --signature ffmpeg.json.sig \
  --certificate ffmpeg.json.crt \
  --output ffmpeg.json.bundle

# 4. Submit PR to registry repository
git add ffmpeg.json ffmpeg.json.bundle
git commit -m "Add ffmpeg shim"
git push origin add-ffmpeg

# 5. CI validates and merges
# 6. CDN automatically updated
```
